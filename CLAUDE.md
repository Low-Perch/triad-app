...
Package manager is **pnpm**. Dictionary curation: `python3 scripts/filter_dict.py` regenerates `crates/triad-core/src/resources/dict.json` (requires `nltk` + `wordfreq`; manual removals go in `scripts/blocklist.txt`). **After any dict change**, re-run `cargo run -p triad-core --bin pin_puzzles -- <end-date>` to regenerate the future tail of `crates/triad-core/src/resources/pinned.json`, then `pnpm wasm:build` (both resources are embedded).
