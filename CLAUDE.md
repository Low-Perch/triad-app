...
- **`crates/triad-core`** — the engine, shared by both targets:
  ...
  - `generator.rs` — puzzle generation from the embedded `resources/dict.json`. Daily puzzles are deterministic: ChaCha8 RNG seeded by puzzle number (days since the 2025-01-01 epoch), with word lists sorted before selection for cross-platform determinism. `now_unix_secs()` panics on WASM — timestamps must be passed in from JS. All timestamps fed to the engine are pre-shifted by the local UTC offset so day boundaries fall at local midnight (wasm shifts via `getTimezoneOffset`, desktop via `chrono::Local`)
  - **Difficulty & fairness**: dailies follow a weekday ramp (`tier_for_puzzle_number`: Mon/Tue easy, Wed–Fri medium, Sat/Sun hard) over the curation-assigned `DictEntry::tier`; random games draw from all tiers. Every generated selection (daily and random) must pass `is_fair_selection`: the key is the *only* same-length chunk completing all three fragments to dictionary words (no alternate valid answer), and at most one fragment is ≤2 letters. Pins regenerated before this guard existed (published dates ≤ #572) may violate it — they're frozen by design
