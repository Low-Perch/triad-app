...
Desktop auto-update: `tauri-plugin-updater` checks `releases/latest/download/latest.json` on launch (`lifecycle.tauri.ts` → update banner in `App.svelte`). Update artifacts are signed in CI with the `TAURI_SIGNING_PRIVATE_KEY` repo secret (empty password); the private key lives ONLY in `~/.tauri/triad-app-v2.key` and that secret — losing it breaks auto-update for existing installs (rotating the pubkey requires users to reinstall once). The pubkey is embedded in `tauri.conf.json`.
